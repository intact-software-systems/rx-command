...
import com.intact.rx.api.cache.Filter;
...
public class CacheSelection<K, V> implements RxSelection<K, V>, RxFilter<K, V> {
    ...
    private final WeakReference<DataCache<K, V>> attachedToCache;
    ...
    public CacheSelection(Object selectionId, Filter<V> filter, DataCache<K, V> attachedToCache) {
        this.selectionId = requireNonNull(selectionId);
    ...
    @Override
    public boolean filter(K key, V newValue, boolean expired) {
        ...
        if (!expired) {
            ...
            boolean addToSelection = filter.apply(newValue, alreadyMember);

            if (addToSelection) {
                Optional<V> previous = localCache.write(key, newValue);
                ...
                if (!alreadyMember) {
                    selectionSubject.onObjectIn(newValue);
                }
                // Has object changed?
                else if (previous.isPresent() && !Objects.equals(newValue, previous.get())) {
                    selectionSubject.onObjectModified(newValue);
                }
    ...
    public boolean detach() {
        return Optional.ofNullable(attachedToCache.get()).map(dataCache -> dataCache.takeSelection(selectionId).isPresent()).orElse(false);
    }

    @Override
    public boolean isAttached() {
        return Optional.ofNullable(attachedToCache.get()).map(dataCache -> dataCache.isSelectionAttached(selectionId)).orElse(false);
    }

    @Override
    public CacheSelection<K, V> onObjectInDo(VoidStrategy1<V> strategy) {
        selectionSubject.onObjectInDo(strategy);
        return this;
    }

    @Override
    public CacheSelection<K, V> onObjectOutDo(VoidStrategy1<V> strategy) {
        selectionSubject.onObjectOutDo(strategy);
        return this;
    }

    @Override
    public CacheSelection<K, V> onObjectModifiedDo(VoidStrategy1<V> strategy) {
        selectionSubject.onObjectModifiedDo(strategy);
        return this;
    }

    @Override
    public CacheSelection<K, V> onDetachDo(VoidStrategy0 strategy) {
